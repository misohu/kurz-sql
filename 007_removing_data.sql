...
-- Pomocou DROP TABLE vieme zmazat tabulku navzdy 
-- nerobte to 
-- DROP TABLE "Track";

-- Mazat vieme aj pomocou prikazu DELETE FROM 
DELETE FROM "Track"
WHERE 
	"AlbumId" = 1;
-- Ked to spustite druhy krat a riadok uz neexistuje nevypise sa chyba
	
-- Pomocou tohto prikazu vieme aj vratit riadok pred jeho zmazanim ... toto sa vyuziva najme pri API backend serveroch
DELETE FROM "Track"
WHERE 
	"AlbumId" = 2
RETURNING *;

-- Toto vymaze vsetky zaznamy ale zanecha tabulku
DELETE FROM "Track";

-- Ak pouzivate auto increment stlpec v tabulke napr pomocou SERIAL ... DELETE FROM zanecha aktualnu hodnotu
...
SELECT * 
FROM car;
-- DELETE FROM pri mazani prechadza kazdy riadok tabulky a uzamika ho

-- Prikaz truncate table robi to iste co delete table
-- Truncate table pocas vykonavania uzamkne tabulku preto je rychlejsi ako delete from ktory zamkne roadok po riadku
-- Truncate table nevrati ziadnu hodnotu .... a vo vacsine SQL databaz resetuje autoincrement v pripade postgresql si mozeme vybrat co sa stane 
DROP TABLE IF EXISTS car;
CREATE TABLE car(
	car_id SERIAL PRIMARY KEY,
	brand VARCHAR(64)
...
INSERT INTO car (brand) VALUES ('BMW');
-- RESTART IDENTITY vie restartovat pk.
...
-- Poznamka do buducnosti, truncate table nespusta on delete trigger
